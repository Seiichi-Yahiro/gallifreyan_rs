...
fn draw_word_and_letter(
    ...
    >,
    mut letter_query: Query<(&Letter, &Radius, &Transform, &mut super::SVGElement), Without<Word>>,
) {
    ...
    while let Some((word_radius, letters, mut word_svg_element)) = word_iter.fetch_next() {
        ...
        while let Some((letter, letter_radius, letter_transform, mut letter_svg_element)) =
